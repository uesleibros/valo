...
pub(crate) fn eval_err(
    ...
    if field.eq_ignore_ascii_case("Description") {
        return Ok(Some(Value::String(Rc::new(
            interpreter.err_description.clone(),
        ))));
    }
    if field.eq_ignore_ascii_case("Source") {
        return Ok(Some(Value::String(Rc::new(interpreter.err_source.clone()))));
    }
    if field.eq_ignore_ascii_case("HelpFile") {
        return Ok(Some(Value::String(Rc::new(
            interpreter.err_help_file.clone(),
        ))));
    }
