...
`types` and `math` are converted. Three tests cover every converted table: a
handler may only name a builtin the registry declares, a table may not register
a name twice, and no two modules may claim the same name — which would otherwise
be a silent choice between two implementations, decided by table order.
...
macro rather than written out thirteen times.
